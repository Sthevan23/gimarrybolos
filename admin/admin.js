/**
 * admin.js — Painel Administrativo Gimarry Bolos
 * Produtos, pedidos, financeiro e configurações (sem impressora)
 */
...
function updateSyncBadge() {
  ...
  if (!el || !badge) return;
  el.textContent = 'Neste aparelho';
  badge.classList.remove('sync-badge--on');
  badge.title = 'Os dados ficam salvos neste navegador.';
}
...
async function refreshOrdersFromCloud() {
  ...
  try {
    ...
    renderDashboard();
    updateSyncBadge();
    showToast('Pedidos atualizados neste aparelho.', 'success');
  } finally {
    if (btn) {
      btn.disabled = false;
...
async function fetchAnalyticsData(period) {
  if (typeof Storage === 'object' && Storage.getAnalytics) {
    return Storage.getAnalytics(period);
  }
  throw new Error('Análises indisponíveis');
}
...
function initButtons() {
  ...
  document.getElementById('btn-publish-catalog')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (ok) showToast('Cardápio atualizado neste aparelho. Abra o site para ver.', 'success');
      else showToast('Não deu para atualizar o cardápio.', 'error');
    } finally {
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-store"></i> Atualizar cardápio no site';
      }
    }
  });
